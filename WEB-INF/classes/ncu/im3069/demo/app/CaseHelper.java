...
public class CaseHelper {
    ...
    public JSONObject getByRequesterIdAndProgress(int id){
        /** 新建一個 Case 物件之 c 變數，用於紀錄每一位查詢回之案件資料 */
        Case c = null;
        /** 儲存JDBC檢索資料庫後回傳之結果，以 pointer 方式移動到下一筆資料 */
        ResultSet rs = null;
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "SELECT `cases`.*, `progress`.`applicant_id`, `progress`.`applicated_time`, `progress`.`finished_time` FROM `cases` LEFT JOIN `progress` on `cases`.`requester_id`=`progress`.`requester_id` AND `cases`.`id`=`progress`.`case_id` WHERE `cases`.`requester_id` = ?";
            ...
            while(rs.next()) {
                ...
                String applicated_time = rs.getString("applicated_time");
                if(applicated_time == null){
                    applicated_time = "未接案";
                }
                String finished_time = rs.getString("finished_time");
                if(finished_time == null){
                    finished_time = "未完成";
                }
                
                /** 將每一筆商品資料產生一名新Case物件 */
                c = new Case(case_id, requester_id, applicant_id, applicated_time, finished_time);
                /** 取出該項案件之資料並封裝至 JSONsonArray 內 */
                jsa.put(c.getData());
            }
    ...
    public JSONObject getByApplicantIdAndProgress(int id){
        /** 新建一個 Case 物件之 c 變數，用於紀錄每一位查詢回之案件資料 */
        Case c = null;
        /** 儲存JDBC檢索資料庫後回傳之結果，以 pointer 方式移動到下一筆資料 */
        ResultSet rs = null;
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "SELECT `cases`.*, `progress`.`applicant_id`, `progress`.`applicated_time`, `progress`.`finished_time` FROM `cases` LEFT JOIN `progress` on `cases`.`requester_id`=`progress`.`requester_id` AND `cases`.`id`=`progress`.`case_id` WHERE `progress`.`applicant_id` = ?";
